...
function buildCheckFormPayloadWithSubject(form) {
  return {
    ...
    basisLink: String(form.basisLink || "").trim(),
...
function PageChecks({ user, currentUser, sessionUser, users, factions, positions, checksMeta, onRefreshChecksMeta }) {
  ...
  useEffect(() => {
    ...
    setMetadataForm({
      ...
      basisLink: detail.check.basisLink || "",
    ...
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={S.pageSubtitle}>Проверки фракций: карточка, участники, отчёты, опросы, сводка, утверждение и передача в Генеральную прокуратуру.</p>
        </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
                <div>
                  ...
                  <div style={{ fontSize: T.body, color: C.textDim, marginTop: 6 }}>{currentCheck.subject} • {currentCheck.basisText || "Основание не указано"}{currentCheck.basisLink && React.createElement("span", null, " • ", React.createElement("a", { href: currentCheck.basisLink, target: "_blank", rel: "noopener noreferrer", style: { color: C.accent } }, "Постановление"))}</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", alignItems: "flex-start", alignSelf: "flex-start" }}>
                  ...
                  {permissions.canDelete && (currentCheck.status !== "approved" || hasSystemAdminAccess(sessionActor)) && canMutate ? <button className="btn-hover" style={btn("danger")} onClick={openDeleteConfirm} disabled={Boolean(actionBusy) || deleteBusy}>Удалить проверку</button> : null}
                </div>
              </div>
...
function CheckGeneralTab({ permissions, metadataForm, setMetadataForm, factions, enabledSubjects, sessionActor, canMutate, handleSaveMetadata, actionBusy, currentCheck }) {
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div>
        <label style={S.label}>Ссылка на постановление</label>
        <input style={S.input} type="text" placeholder="https://..." value={metadataForm.basisLink || ""} onChange={e => setMetadataForm(prev => ({ ...prev, basisLink: e.target.value }))} disabled={isApproved || !permissions.canEditMetadata || !canMutate} />
        {metadataForm.basisLink && <a href={metadataForm.basisLink} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: C.accent, marginTop: 4, display: "inline-block" }}>Открыть постановление</a>}
      </div>
      <div>
...
function CheckCreateModal({ createForm, setCreateForm, factions, enabledSubjects, sessionActor, createParticipantCandidates, handleCreate, actionBusy, onClose, users }) {
  return (
    <Modal onClose={onClose} maxWidth={860}>
      ...
      <label style={{ ...S.label, marginTop: 16 }}>Ссылка на постановление</label>
      <input style={S.input} type="text" placeholder="https://..." value={createForm.basisLink || ""} onChange={e => setCreateForm(prev => ({ ...prev, basisLink: e.target.value }))} />
